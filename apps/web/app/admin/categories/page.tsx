...
export default function AdminCategoriesPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      updateCategory({ id: editingId, ...formData }, { onSuccess });
    } else {
  ...
  const handleDelete = (id: string) => {
    ...
    deleteCategory(id, { onSuccess: () => toast.success("Category deleted") });
  };
  ...
  const totalProducts = categories.reduce((s, c) => s + ((c as Category & { _count?: { products: number } })._count?.products || 0), 0);
  ...
  return (
    <>
      <style>{`
                ...
                .ac-add-btn { display:inline-flex; align-items:center; gap:8px; padding:13px 24px; border-radius:6px; background:var(--ink); color:#fff; border:none; cursor:pointer; font-family:'DM Sans',sans-serif; font-size:12px; font-weight:500; letter-spacing:.1em; text-transform:uppercase; transition:background .2s, transform .15s; }
                ...
                .ac-input { width:100%; height:44px; padding:0 14px; border:1.5px solid var(--border); border-radius:6px; background:var(--paper); color:var(--ink); font-family:'DM Sans',sans-serif; font-size:14px; font-weight:400; outline:none; transition:border-color .2s; }
      ...
      <div className="ac-wrap">
              ...
              <form onSubmit={handleSubmit} className="ac-form">
                ...
                <div className="ac-form-grid">
                  <div>
                    ...
                    <input required className="ac-input" placeholder="e.g. Running" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                  </div>
                  <div>
                    <label className="ac-label">Description (optional)</label>
                    <input className="ac-input" placeholder="Brief description" value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} />
                  </div>
                </div>
        ...
        <div className="ac-grid">
          {loading ? (
            ...
            (categories as Array<Category & { _count?: { products: number } }>).map((c, i) => (
              <motion.div key={c.id} layout className="ac-card" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
                <div>
